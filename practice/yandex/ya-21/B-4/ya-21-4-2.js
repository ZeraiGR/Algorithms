...
const voices = fileContent.trim().split('\n');

const map = new Map();

voices.forEach(voice => {
	const [name, count] = voice.trim().split(' ');
	map.set(name, (map.get(name) ?? BigInt(0)) + BigInt(count));
});

let keys = [...map.keys()].sort();

let res = keys.map(key => `${key} ${map.get(key)}`);
