...
lines.forEach((line, i, arr) => {
	if (!isNaN(+line.trim())){
		if (+line === 0) {
			messages.set({name: arr[i+1].trim(), isTopic: true}, +line);
		} else {
			messages.set({name: arr[i+1].trim(), isTopic: false}, +line);
		}
	}
});
...
function getTopic(topics, key) {
	if (topics[key -1][0].isTopic) {
		return topics[key -1][0];
	} else {
		return getTopic(topics, topics[key -1][1]);
	}
}

[...messages.entries()].forEach(([name, key], i, arr) => {
	if (key === 0) {
		topics.set(name, 0);
	} else {
		let topic = getTopic(arr, key);
		topics.set(topic, topics.get(topic) + 1);
	}
});

let max = Math.max(...topics.values());

let res = [...topics.entries()].filter(([obj, val]) => val === max).map(el => el[0].name);

fs.writeFileSync("output.txt", res[0]);